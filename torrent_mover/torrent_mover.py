...
def get_remote_size_rsync(sftp_config, remote_path):
    ...
    """
    # Mock for testing
    if os.environ.get("JULES_TEST_MODE"):
        print(f"MOCK_GET_SIZE_PATH: {remote_path}")
        return 1024
...
def transfer_content_rsync(sftp_config, remote_path, local_path, job_progress, parent_task_id, overall_progress, overall_task_id, dry_run=False):
    ...
    # Mock for testing
    if os.environ.get("JULES_TEST_MODE"):
        print(f"MOCK_TRANSFER_PATH: {remote_path}")
...
def analyze_torrent(torrent: Torrent, source_config, live_console):
    ...
    if client_path_prefix and transfer_path_prefix:
        if torrent.content_path.startswith(client_path_prefix):
            # A more robust way to handle path replacement, especially for root paths.
            if client_path_prefix == '/':
                # If the client path is root, the relative path is the content path, minus the leading slash.
                relative_path = torrent.content_path.lstrip('/')
            else:
                relative_path = os.path.relpath(torrent.content_path, client_path_prefix)

            remote_content_path = os.path.join(transfer_path_prefix, relative_path)
...
def main():
    ...
    try:
        ...
        try:
            if os.environ.get("JULES_TEST_MODE"):
                # Mock the clients and torrents for testing
                from unittest.mock import MagicMock
                source_client = MagicMock()
                source_client.get_torrents_to_move.return_value = [
                    Torrent(hash='123', name='My.Awesome.Torrent', category='move-me', content_path='/data/torrents/My.Awesome.Torrent', size=1024, added_on=0, progress=1, state='completed', raw_torrent=None)
                ]
                dest_client = MagicMock()
                category_to_move = "move-me"
                size_threshold_gb = None
                source_config = config['MySeedbox']
                eligible_torrents = source_client.get_torrents_to_move(category_to_move, size_threshold_gb)
                # This is needed for the test to have a dest_config
                dest_config = config['HomeServer']
            else:
