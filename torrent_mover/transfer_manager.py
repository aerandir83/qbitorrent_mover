...
from .ui import BaseUIManager, UIManagerV2 as UIManager
...
def _transfer_content_rsync_upload_from_cache(
    ...
    ui.log_transfer(torrent_hash, f"Starting rsync upload from cache for '{file_name}'")
    ...
    if dry_run:
        ...
        if total_size > 0:
            ui.update_torrent_progress(torrent_hash, total_size, transfer_type)
        ui.log_transfer(torrent_hash, f"[DRY RUN] Completed rsync upload for {file_name}")
        return
        ...
        try:
                ...
                while True:
                    ...
                    if byte == b'\r' or byte == b'\n':
                        if line_buffer:
                            ...
                            ui.log_transfer(torrent_hash, f"[DEBUG] Raw rsync line: {line}")
                            ...
                            if match:
                                ...
                                ui.log_transfer(torrent_hash, f"[DEBUG] Parsed bytes: {current_transferred_bytes}")

                                transferred_delta = current_transferred_bytes - last_transferred_bytes
                                if transferred_delta > 0:
                                    ui.update_torrent_progress(
                                        torrent_hash,
                                        transferred_delta,
                                        transfer_type
                                    )
            ...
            if process.returncode == 0 or process.returncode == 24:
                if total_size > 0 and last_transferred_bytes < total_size:
                    ...
                    ui.update_torrent_progress(torrent_hash, remaining, transfer_type)

                logging.info(f"Rsync upload from cache completed for '{file_name}'.")
                ui.log_transfer(torrent_hash, f"[green]Rsync upload complete: {file_name}[/green]")
                return
...
def transfer_content_rsync(
    ...
    ui.log_transfer(torrent_hash, f"Starting rsync transfer for '{rsync_file_name}'")
    ...
    for attempt in range(1, MAX_RETRY_ATTEMPTS + 1):
        ...
        try:
            ...
            if dry_run:
                ...
                if total_size > 0:
                    ui.update_torrent_progress(torrent_hash, total_size, transfer_type)
                ui.log_transfer(torrent_hash, f"[DRY RUN] Completed rsync transfer for {rsync_file_name}")
                return
            ...
            if process.returncode == 0 or process.returncode == 24:
                ...
                if total_size > 0 and last_transferred_bytes < total_size:
                    ui.update_torrent_progress(torrent_hash, total_size - last_transferred_bytes, transfer_type)
                ...
                if os.path.exists(local_path):
                    ...
                    try:
                        ...
                        if size_diff_percent < 0.1:
                            ...
                            ui.log_transfer(torrent_hash, f"Rsync transfer completed and verified for {rsync_file_name}")
                            return
                        else:
