    ...
    try:
        ...
        with pool.get_connection() as (sftp, ssh):
            ...
            if remote_size == 0:
                ...
                progress_callback(0) # Mark as done
                return
            ...
            if local_size == remote_size:
                if not force_integrity_check:
                    logging.info(f"Skipping (exists and size matches): {file_name}")
                    progress_callback(remote_size) # Mark full size as done
                    return
                else:
            ...
            with sftp.open(remote_file, 'rb') as remote_f_raw:
                ...
                with open(local_path, mode) as local_f:
                    while True:
                        ...
                        progress_callback(chunk_len) # Report INCREMENT
...
def transfer_content_sftp(
    ...
    def worker_progress_callback(increment: int):
        nonlocal total_transferred
        with progress_lock:
            total_transferred += increment
            # Calculate overall progress
            _update_transfer_progress(torrent_hash, total_transferred / total_size if total_size else 0, total_transferred, total_size, status_text="Downloading")
        ...
        while True:
            ...
            try:
                _sftp_download_file_worker(
                    ...
                    worker_progress_callback, force_integrity_check
                )
